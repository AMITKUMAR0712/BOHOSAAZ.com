...
type Action = "approve" | "reject" | "schedule_pickup" | "mark_picked";

export default function VendorReturnDetailPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    setLoading(true);
    const res = await fetch(`/api/vendor/returns/${params.id}`, { credentials: "include" });
    ...
    setRr(d.data?.returnRequest || null);
    setLoading(false);
  }
  ...
  async function doAction(action: Action, payload: Record<string, unknown> = {}) {
    setMsg(null);
    setBusy(action);
    const res = await fetch(`/api/vendor/returns/${params.id}`, {
      method: "PATCH",
        ...
        <>
          <div className="rounded-2xl border overflow-hidden">
            ...
            <div className="p-4 grid gap-2 text-sm">
              ...
              <div>
                Order #{rr.order.id} • {new Date(rr.order.createdAt).toLocaleString()}
              </div>
              <div>
