...
export function VendorApplyClient() {
  ...
  async function submit() {
    ...
    try {
      const payload = {
        mode: editMode || vendorStatus === "APPROVED" ? "edit" : "apply",
        shop: {
      ...
      if (!res.ok) {
        ...
        if (Array.isArray(json?.issues)) {
          for (const issue of json.issues) {
            ...
            if (key) next[key] = issue.message || String(issue);
          }
        }
        ...
        const issueText = Array.isArray(json?.issues)
          ? json.issues.map((issue: any) => issue.message || String(issue)).join("; ")
          : null;

        setMsg(
          `❌ ${json?.error || "Submit failed"}${issueText ? `: ${issueText}` : ""}`
        );
        return;
      }

      setMsg(
        editMode || vendorStatus === "APPROVED"
          ? "✅ Changes submitted for admin re-approval (PENDING)"
      ...
      if (editMode || vendorStatus === "APPROVED") {
        router.replace("/account/vendor-status");
      } else {
  ...
  if (vendorStatus === "APPROVED" && !editMode) {
    return (
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Link href="/account/vendor-apply?edit=1">
            ...
            <p className="text-xs text-muted-foreground">
              Editing sends your application back for admin approval. Selling access pauses until re-approved.
            </p>
  ...
  return (
    <div className="mx-auto max-w-3xl p-6 md:p-10">
      <Card>
        <CardHeader>
          <CardTitle>{editMode || vendorStatus === "APPROVED" ? "Edit Vendor Application" : "Become a Vendor"}</CardTitle>
          <CardDescription>
            {editMode || vendorStatus === "APPROVED"
              ? "Update shop + KYC details. Submitting will request admin re-approval."
              : "Submit shop info + KYC. Admin approval required."}
          </CardDescription>
        </CardHeader>
            ...
            <div>
              ...
              <div className="mt-3 grid gap-3">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <div>
                    <label className="text-sm">Store Banner (optional)</label>
                    <div className="mt-1 flex items-center gap-3">
                      <div className="h-12 w-24 overflow-hidden rounded-(--radius) border bg-muted/20">
                        {form.bannerUrl ? <img src={form.bannerUrl} alt="Banner" className="h-full w-full object-cover" /> : null}
                      </div>
                      <input
                        type="file"
                        accept="image/*"
                        disabled={busyKey === "bannerUrl" || submitting}
                        onChange={(e) => {
                          const f = e.target.files?.[0];
                          if (f) void uploadField(f, "bannerUrl", "vendor_logo");
                        }}
                        className="block w-full text-sm"
                      />
                    </div>
