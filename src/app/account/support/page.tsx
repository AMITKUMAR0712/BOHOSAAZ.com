    ...
    <div className="grid gap-6">
      ...
      <div className="rounded-2xl border overflow-hidden">
        ...
        <div className="p-4 grid gap-3 max-w-2xl">
          ...
          <SupportAttachmentPicker attachments={attachments} onChange={setAttachments} onError={toast.error} />
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      {last?.message ? (
                        <div className="mt-2 text-xs text-gray-600 line-clamp-1">Last: {last.message}</div>
                      ) : null}
                    </div>
                    <div className="text-xs text-gray-600">{new Date(t.updatedAt).toLocaleString()}</div>
                  </div>
                </Link>
              );
            })}
      ...
      <div className="text-xs text-gray-600">
        Looking for returns? Go to <Link className="underline" href="/account/returns">Returns & refunds</Link>.
      </div>
    </div>
  );
}
