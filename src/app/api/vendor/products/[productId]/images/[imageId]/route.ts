...
export async function DELETE(
  ...
  const rlKey = `vendor:product:image:delete:${req.headers.get("x-forwarded-for") || "ip"}`;
  ...
  const img = await prisma.productImage.findUnique({ where: { id: imageId }, select: { id: true, productId: true, isPrimary: true } });
  if (!img || img.productId !== productId) return Response.json({ error: "Image not found" }, { status: 404 });

  await prisma.productImage.delete({ where: { id: imageId } });

  // if primary deleted, auto set another primary
  ...
  await audit({
    ...
    action: "VENDOR_PRODUCT_IMAGE_DELETE",
    entity: "Product",
    entityId: productId,
    meta: { imageId },
