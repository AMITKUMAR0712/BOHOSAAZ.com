...
export async function POST(
  ...
  const updated = await prisma.userTicket.update({
    where: { id: ticketId },
    data: { status: parsed.data.status as UserTicketStatus },
  ...
  await audit({
    ...
    action: "ADMIN_USER_TICKET_STATUS_SET",
    entity: "UserTicket",
    entityId: updated.id,
  ...
  return jsonOk({ ticket: { ...updated, updatedAt: updated.updatedAt.toISOString() } });
}
