...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (variantId && !pickedVariant) return Response.json({ error: "Variant not found" }, { status: 404 });
    ...
    return Response.json({ ok: true, orderId: order.id, total });
  } catch (e: unknown) {
    console.error("[api/cart/buynow] failed:", e);
